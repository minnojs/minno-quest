...
In order to run a MinnoJS task, you need to have the study files on the internet.
In addition you need to have a file that tells the user's browser 
how to run the experiment and which experiment to run.

Having the files "on the internet" is tantamount to putting it on a [server](https://developer.mozilla.org/en-US/docs/Learn/Common_questions/What_is_a_web_server).
You can checkout a simple php server [here](https://github.com/minnojs/simple-minno-server),
or if you do not need to save any data, you can use any server that you like 
(such as [wamp](https://www.wampserver.com/en/) or [mamp](https://www.mamp.info/)).
You can see [this](https://minnojs.github.io/minnojs-blog/local/) blog post, that explains how to run a local server for efficient testing of MinnoJS tasks.

### Setting up

Here, we focus not on setting up your server, but rather on telling the browser how to run your study.
A MinnoJS study is run by accessing a web page that activates the experiment scripts.
Web pages are created using a markup language called [HTML](https://developer.mozilla.org/en-US/docs/Web/HTML).
Below is an HTML page that is built to launce a MinnoJS study.
The important line to notice is the following:

```HTML
<div pi-manager="study/mgr.js"></div>
```

This is the line that defines the url where MinnoJS can find your study (in this case: `study/mgr.js`).
It is the only line that you are likely to need to change.
The code running MinnoJS itself is automatically downloaded from a central repository.
And most of the local modification can be done from within the study scripts themselves.
...
### Advanced usage
There are some cases where you may want to modify this template further.
Though all of them depond on having a dynamic server (i.e. a server than can dynamically change the html).
These are generally advanced options, if you don't understand the terminology, please consult an IT person.

The most obvious change is to let the server dynamicaly choose the task to be run.
Additionally, it is possible to pass information to the player through the use of JavaScript.
The way that we do this, is by setting the data into the [global object](./variables.html#global).

For example, the following addition creates a global object with two keys:
`userName` is an arbitrary value that would be used within the study (maybe as a greeting?).
`$meta` is a special keyword object that will be added to all posts to the server (see [here](./variables.html#meta).
