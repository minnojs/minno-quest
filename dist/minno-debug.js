...
var css_248z = ".minno-console{position:fixed;left:0;top:0;height:100%;width:30%;min-width:300px;background:#fff;opacity:.8}.minno-console-logs{height:calc(100% - 34px);overflow-y:scroll}.minno-console.minno-console-hide{width:-moz-min-content;width:-webkit-min-content;min-width:auto;height:auto}.minno-console:hover{opacity:.95}";
...
var Console = {
    oninit: function(vnode){
        ...
        window.addEventListener('message', function receiveMessage(event){
            if (event.data.type === 'kill-console') mithril.mount(root, null);
            $messages(event.data);
            mithril.redraw();
        },false);
        ...
        return mithril('.minno-console', {class: show ? '' : 'minno-console-hide'}, [
            mithril('.input-group', [
                ...
                state.$show() && mithril('select.form-control', {title: 'Select log level',onchange:mithril.withAttr('value', state.$level), value:state.level}, [
                    ...
                    mithril('option', {value:2, selected:level==2}, 'Warn'),
                    mithril('option', {value:9, selected:level==9}, 'All')
                ]),        
                mithril('span.input-group-btn',[
                    mithril('button.btn.btn-default',{
                        title: show ? 'Hide console' : 'Show console',
                        onclick: toggleShow
                    }, mithril('strong', show ? '<<' : '>>'))
                ])
            ]),
            show && mithril(Scroller, {className:'.minno-console-logs'}, state.$logs().filter(levelFilter).map(function(r){ return mithril(consoleRow, r); }))
        ]);
    ...
    view: function(vnode){
        ...
        return !state.hide && mithril('.panel', {class: panelClasses[levelMap[row.type] || 5] }, [
            mithril('.panel-heading', [
                ...
                mithril('button', {class:'close', style:'margin-left:5px;', onclick:toggle(state,'hide')}, mithril.trust('&times;')),
                (row.context || row.rows) && mithril('button', {
                    class:'close', 
                    onclick: toggle(state,'open')
                }, mithril.trust(state.open ? '&minus;' : '&plus;'))
            ]),

            state.open && [
                row.context && mithril('.panel-body', [
                    mithril('strong', 'Context:'),
                    mithril('pre', syntaxHighlight(row.context))
                ]),

                row.rows && mithril('list-group', row.rows
                    .map(function(r){return [].concat(r);}) // ensure each row is an array
                    .map(function(r){
                        return mithril('.list-group-item', r
                            .map(syntaxHighlight)
                            .map(function(v){return mithril('span', {style:'margin-right:7px;'},v);})
                        );
                    })
                )
            ]
...
function syntaxHighlight(json) {    
    ...
    return mithril.trust(json.replace(/("(\\u[a-zA-Z0-9]{4}|\\[^u]|[^\\"])*"(\s*:)?|\b(true|false|null)\b|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?)/g, function (match) {
        var style = _number;
        if (/^"/.test(match)) {
            if (/:$/.test(match)) {
                style = _key;
...
var Scroller = {
    ...
    view: function(vnode){
        return mithril(vnode.attrs.className || 'div', vnode.children);
    }
};

mithril.mount(root, Console);
